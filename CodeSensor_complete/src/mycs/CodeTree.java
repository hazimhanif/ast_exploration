...
public class CodeTree
{
    ...
    public void print(PrintWriter pwlocal, String filename) throws IOException
    {
        ...
        try{
            ...
            String local = NodePrinter.stmnt.substring(0, NodePrinter.stmnt.length()-1)+"]";
            pwlocal.println(local.replaceAll("\\r", "").replaceAll("\\n", ""));
        }catch(Exception e){
...
class NodePrinter
{
    ...
    public static boolean printCSV(CodeTreeNode node)
    {
	String type;
	String codeStr;
	...
	if(type.equals("LEAF_NODE")){
	    //System.out.print("[\'" + codeStr.replaceAll("\'","\\\\\"") + "\']");
            stmnt  += "[\'" + codeStr.replaceAll("\'","\\\\\"") + "\']"; 
	    return false;
	} 

        if (type.equals("STATEMENTS")||type.equals("SOURCE_FILE")){	
	    int numberOfChildren = node.children.size();
	    for(int i = 0; i < numberOfChildren; i++){
		CodeTreeNode child = (CodeTreeNode) node.children.get(i);
		if (!printCSV(child)) stmnt += ","; //System.out.println("");
	    }
	    return true;
	}
	...
	for(int i = 0; i < numberOfChildren; i++){
	    CodeTreeNode child = (CodeTreeNode) node.children.get(i);
	    if (child.codeStr.equals("{")||ASTNodeWrapper.getType(child.astNode).equals("STATEMENTS")){
		//System.out.println("]");
                stmnt += "],";
		printCSV(child); 
		if (child.codeStr.equals("{")){
		    //System.out.println("");
                    stmnt += ",";
		    printCSV((CodeTreeNode) node.children.get(i+1));
		    printCSV((CodeTreeNode) node.children.get(i+2)); 
		    //System.out.println("");
                    stmnt += ",";
		}		
		return true;
